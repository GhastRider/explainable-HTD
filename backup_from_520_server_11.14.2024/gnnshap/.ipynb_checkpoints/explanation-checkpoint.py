...
class GNNShapExplanation:
    ...
    def plot_graph(self, topk: int = 25, save_path: str = None, pos=None, show_scores: bool = False,
                   return_pos: bool = False, show: bool = True, name_to_index = None, font_size: int = 12) -> None:
        ...
        if name_to_index is not None:
            labels = {n: name_to_index[n] for n in nodes if n in name_to_index}

        else:
            labels = {n: f'{n}' for n in nodes}  # 默认使用节点索引作为标签
        # 绘制图形
        ...
        #     node_labels = {n: f'{n}' for n in nodes}  # 默认使用节点索引
        # else:
        #     node_labels = {n: node_names.get(n, n) for n in nodes}  # 使用自定义名称
